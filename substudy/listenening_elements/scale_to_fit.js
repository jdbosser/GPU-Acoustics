...
const near = 0.01;
const far = 100;
let mListeningCamera = new ListeningCamera(
        width / - 2, width / 2, 
        height / 2, height / - 2);
mListeningCamera.up.set(0,0,1);     
mListeningCamera.position.set(0,3,0);
mListeningCamera.lookAt(0,0,0);
mListeningCamera.near = 0;
mListeningCamera.updateProjectionMatrix();

mListeningCamera.addListener({property: "position", callback: () => console.log("it works!")})

renderer.render(scene, mListeningCamera);
